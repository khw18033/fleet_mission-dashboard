...
| `target_stations` | 선택 | 미션을 **브로드캐스트할 기지국(edge) 노드** 이름. 스크립트 옵션으로 덮어쓸 수 있음. 비우면 전체 기지국이 브로드캐스트 |
...
채웁니다. 기지국 broadcaster는 edge 노드에서 실행되며 자신의 `STATION_ID`(= 노드 이름)가
타깃에 포함될 때만 미션을 브로드캐스트합니다. 따라서 **타깃은 기지국(edge) 노드**여야
하며, `--node-label node-role=edge` 또는 특정 edge 노드 이름으로 지정합니다. 로봇 노드를
지정하면 어떤 broadcaster와도 매칭되지 않아 브로드캐스트되지 않습니다.

> 검증: `target_stations=[edge-1]` 인 미션 배포 시 `STATION_ID=edge-1` broadcaster는
> 브로드캐스트하고, `STATION_ID=edge-2` broadcaster는 skip함을 확인했습니다.
